...
class FeatureBasicEntityTest {
    ...
    /**
     * CONCISE REFACTOR: Generic filler for any Feature builder.
     * Use: fill(FeatureBasicEntity.builder()) or fill(FeatureScalingPriceEntity.builder())
     */
    ...
    class ValidationTests {
        ...
        void shouldPreventSelfIncompatibility() {
            ...
            // We use the fill() method but override the UuId and Incompatibles to trigger the guard
            var builder = fill(FeatureBasicEntity.builder())
                    .featureUuId(selfId)
    ...
    class ImmutabilityTests {
        ...
        void shouldBeImmuneToExternalInputMutation() {
            ...
            FeatureBasicEntity entity = fill(FeatureBasicEntity.builder())
            ...
            assertThat(entity.getIncompatibleFeatures()).hasSize(initialSize);
        }

        @Test
        void shouldReturnUnmodifiableSet() {
            FeatureBasicEntity entity = fill(FeatureBasicEntity.builder()).build();
            assertThatThrownBy(() -> entity.getIncompatibleFeatures().add(UuIdVO.generate()))
                    .isExactlyInstanceOf(UnsupportedOperationException.class);
        }
    }

    @Nested
    @DisplayName("3. Identity Logic")
    class IdentityTests {
        ...
        void shouldObeyIdentityEqualityRules() {
            ...
            FeatureBasicEntity entity1 = fill(FeatureBasicEntity.builder()).featureUuId(sharedId).featureName(NameVO.from("Aa-Test")).build();
            FeatureBasicEntity entity2 = fill(FeatureBasicEntity.builder()).featureUuId(sharedId).featureName(NameVO.from("Bb-Test")).build();
    ...
    @Nested
    @DisplayName("4. Semantic Helpers")
    class SemanticHelperTests {

        @Test
        void testIsVariantSpecific() {
            FeatureBasicEntity variantEntity = fill(FeatureBasicEntity.builder()).featVariantId(VARIANT_UUID).build();
            FeatureBasicEntity genericEntity = fill(FeatureBasicEntity.builder()).featVariantId(null).build();
        ...
        void testVariantIdOptionalWrapping() {
            // Concise refactor using the fill factory
            FeatureBasicEntity entity = fill(FeatureBasicEntity.builder())
    ...
    class BuilderTests {
        ...
        void testRecursiveGenericBuilder() {
            // Demonstration of chaining fill() with subclass-specific logic
            FeatureBasicEntity entity = fill(FeatureBasicEntity.builder())
                    .featVariantId(VARIANT_UUID)
                    .build();
