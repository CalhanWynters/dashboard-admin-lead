...
import com.github.calhanwynters.dashboard_admin_lead.systemproducts.core.application.usecases.types.command.TypesUpdateBusUuIdCommand;
...
/**
 * Handler for updating a Product Type's Business UUID.
 * Orchestrates ownership changes and event emission via TypesAggregate for SOC 2 integrity.
 */
@Service
public class TypesUpdateBusUuIdHandler {

    private final TypesRepository repository;

    public TypesUpdateBusUuIdHandler(TypesRepository repository) {
        this.repository = repository;
    }

    @Transactional
    public TypesDTO handle(TypesUpdateBusUuIdCommand command) {
        ...
        // Triggers updateBusinessUuId(), validation, and TypeBusinessUuIdChangedEvent
        aggregate.updateBusinessUuId(
                command.data().toTypesBusinessUuId(),
