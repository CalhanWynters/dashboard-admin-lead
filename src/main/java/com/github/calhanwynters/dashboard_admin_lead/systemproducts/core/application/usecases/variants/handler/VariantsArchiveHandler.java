...
import com.github.calhanwynters.dashboard_admin_lead.systemproducts.core.application.usecases.variants.command.VariantsArchiveCommand;
...
/**
 * Handler for archiving a Variant.
 * Orchestrates the transition to a read-only state via VariantsAggregate for SOC 2 compliance.
 */
@Service
public class VariantsArchiveHandler {

    private final VariantsRepository repository;

    public VariantsArchiveHandler(VariantsRepository repository) {
        this.repository = repository;
    }

    @Transactional
    public VariantsDTO handle(VariantsArchiveCommand command) {
        ...
        // Triggers lifecycle state change and VariantArchivedEvent registration
