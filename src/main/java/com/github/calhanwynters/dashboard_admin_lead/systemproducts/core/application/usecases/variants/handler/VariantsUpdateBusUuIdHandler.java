...
import com.github.calhanwynters.dashboard_admin_lead.systemproducts.core.application.usecases.variants.command.VariantsUpdateBusUuIdCommand;
...
/**
 * Handler for updating a Variant's Business UUID.
 * Orchestrates ownership changes and event emission via VariantsAggregate for SOC 2 integrity.
 */
@Service
public class VariantsUpdateBusUuIdHandler {

    private final VariantsRepository repository;

    public VariantsUpdateBusUuIdHandler(VariantsRepository repository) {
        this.repository = repository;
    }

    @Transactional
    public VariantsDTO handle(VariantsUpdateBusUuIdCommand command) {
        ...
        // Triggers updateBusinessUuId(), validation, and VariantsBusinessUuIdChangedEvent
        aggregate.updateBusinessUuId(
                command.data().toVariantsBusinessUuId(),
