...
public record PriceFixedPurchase(Money fixedPrice) implements SimplePurchasePricing {
    ...
    public PriceFixedPurchase {
        ...
        if (fixedPrice.amount().compareTo(BigDecimal.ZERO) <= 0) {
            throw new IllegalArgumentException("Fixed price must be greater than zero.");
        }
    }
