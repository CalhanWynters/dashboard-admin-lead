...
import com.github.calhanwynters.dashboard_admin_lead.systemproducts.core.application.usecases.types.command.TypesSoftRestoreCommand;
...
/**
 * Handler for restoring a soft-deleted Product Type.
 * Orchestrates the recovery transition via TypesAggregate for SOC 2 lifecycle accountability.
 */
@Service
public class TypesSoftRestoreHandler {

    private final TypesRepository repository;

    public TypesSoftRestoreHandler(TypesRepository repository) {
        this.repository = repository;
    }

    @Transactional
    public TypesDTO handle(TypesSoftRestoreCommand command) {
        // 1. Map raw UUID to Domain Technical ID
        TypesUuId typesUuId = new TypesUuId(UuId.fromString(command.targetUuId().toString()));

        // 2. Retrieve existing aggregate (including soft-deleted records)
        ...
        // Triggers state transition and emits TypeRestoredEvent
