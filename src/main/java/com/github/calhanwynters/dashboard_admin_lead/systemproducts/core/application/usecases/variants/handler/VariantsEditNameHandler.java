...
import com.github.calhanwynters.dashboard_admin_lead.systemproducts.core.application.usecases.variants.command.VariantsRenameCommand;
...
/**
 * Handler for editing a Variant's Name.
 * Orchestrates identity updates and event emission via VariantsAggregate for SOC 2 integrity.
 */
@Service
public class VariantsEditNameHandler {

    private final VariantsRepository repository;

    public VariantsEditNameHandler(VariantsRepository repository) {
        this.repository = repository;
    }

    @Transactional
    public VariantsDTO handle(VariantsRenameCommand command) {
        ...
        // Triggers rename(), VariantsBehavior validation, and VariantRenamedEvent
        aggregate.rename(
                command.data().toVariantsName(),
