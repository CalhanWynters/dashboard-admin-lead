...
/**
 * DTO for updating a TypeList's Business UUID.
 * Maps raw input to hardened Business Identifiers and Actor context.
 */
public record TypeListUpdateBusUuIdDTO(
        String newBusinessUuid,
        String actorId,
        Set<String> actorRoles
) {
    /**
     * Maps the raw string into a hardened TypeListBusinessUuId.
     * Triggers DomainGuard validation for UUID syntax and length.
     */
    public TypeListBusinessUuId toTypeListBusinessUuId() {
        return new TypeListBusinessUuId(UuId.fromString(newBusinessUuid));
    }
