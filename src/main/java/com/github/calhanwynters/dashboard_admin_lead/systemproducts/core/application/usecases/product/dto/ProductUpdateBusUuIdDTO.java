...
/**
 * DTO for updating a Product's Business UUID.
 * Maps raw input to hardened Business Identifiers and Actor context.
 */
public record ProductUpdateBusUuIdDTO(
        String newBusinessUuid,
        String actorId,
        Set<String> actorRoles
) {
    /**
     * Maps the raw string into a hardened ProductBusinessUuId.
     * Triggers DomainGuard validation for UUID syntax and length.
     */
    public ProductBusinessUuId toProductBusinessUuId() {
        return new ProductBusinessUuId(UuId.fromString(newBusinessUuid));
    }
